...
| `LLMMAN_IGPU_ENABLE` | How the Vulkan probe treats integrated GPUs. Unset (the default): an iGPU is used only when no discrete GPU (and no CUDA/ROCm) was found, so an iGPU-only laptop/APU gets the Vulkan build rather than CPU. Truthy (`1`/`true`/`yes`/`on`): the iGPU always counts, even alongside a discrete GPU, and its memory is added to the VRAM total. Falsy (`0`/`false`/`no`/`off`): integrated GPUs are never used. |
