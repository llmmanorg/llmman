...
| `LLMMAN_SAFETENSORS_ENGINE` | Which engine serves safetensors models: `vllm` or `sglang` (case-insensitive). Unset (or empty) leaves the choice to llmman: `mlx_lm.server` on Apple Silicon (installed when the daemon starts if not on `PATH`; see [backends.md](backends.md#mlx-apple-silicon)), `vllm` otherwise; `vllm` forces vLLM even there. With a container runtime, `sglang` selects the `lmsysorg/sglang` image (pinned by `--sglang-version`). Any other value is reported once and ignored. See [backends.md](backends.md#sglang). |
| `LLMMAN_VLLM_ARGS` | Extra arguments appended to every `vllm serve` llmman starts (plain and `--omni`, local or in a container), split on whitespace — no quoting, so an argument cannot contain a space. For the engine's own knobs llmman has no flag for: `--dtype bfloat16 --enforce-eager`, `--tp 2`, `--quantization awq`. They come after llmman's own `--served-model-name`/`--max-model-len`, so on vLLM's argparse a repeated flag here wins. |
| `LLMMAN_MLX_LM_VERSION` | Exact [`mlx-lm`](https://pypi.org/project/mlx-lm/) release llmman installs into its own environment on Apple Silicon (`0.31.3`); unset takes PyPI's current one. Read only while that environment is first created; an `mlx_lm.server` already on `PATH` is used as-is regardless. |
| `LLMMAN_SGLANG_ARGS` | The same for every `sglang serve` / `sglang.launch_server` llmman starts: `--disable-cuda-graph`, `--tp 2`, `--quantization mlx_q4`, ... |
