...
| `LLMMAN_SAFETENSORS_ENGINE` | Which engine serves safetensors models: `vllm` or `sglang` (case-insensitive). Unset (or empty) leaves the choice to llmman: `mlx_lm.server` on Apple Silicon (installed on first use if not on `PATH`; see [backends.md](backends.md#mlx-apple-silicon)), `vllm` otherwise; `vllm` forces vLLM even there. With a container runtime, `sglang` selects the `lmsysorg/sglang` image (pinned by `--sglang-version`). Any other value is reported once and ignored. See [backends.md](backends.md#sglang). |
