...
| `LLMMAN_HYBRID_PII` | The [privacy gate](providers.md#the-privacy-gate) on a [hybrid model pair](providers.md#hybrid-model-pairs): a request whose body carries a confirmed identifier (email address, phone number, Luhn-valid card number, IBAN, US SSN, routable IP address, issuer-prefixed credential) stays on the local half, and is not retried on the hosted one either. `off`/`0`/`false`/`no` turns it off; anything else, including unset and unreadable, leaves it on. Only a pair's requests are scanned. |
