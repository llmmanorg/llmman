...
rather than authenticating anyone, so on a shared machine prefer an
integration that sends its own key. `cline`, `kimi`, `copilot` and
`openclaw` can't be used with `--provider` at all: the first two pick
their own model rather than taking llmman's, `copilot` has no way to
send a key, and `openclaw` only takes a model during first-run
onboarding.
